...
class ResponseBot {
  constructor() {
    ...
    this.ACTIONS = {
      ...
      swing: () => {
        ...
        if (Math.random() >= 0.6)
          Client.scheduleTask(Math.round(Utils.getRandomInRange(800, 2500) / 50), () => {
            if (!Client.isInGui() && !Client.isInChat()) Player.getPlayer().func_71038_i()
          })
      },
      ...
      chat: () => {
        if (!this.MESSAGES?.length) {
          ChatUtils.sendDebugMessage("No response messages found!")
  ...
  fetchResponse(surroundings) {
    try {
      ...
      ChatUtils.sendCustomMessage("AutoVegetable", `Error occurred during response generation: ${e}`)
      return null
    }
  }
