...
global.modules.push(
  new ConfigModuleClass(
    ...
    [
      ...
      new SettingSelector("Mining Target", 1, ["Gold", "Mithril", "Gemstone", "Ore"]),
    ],
...
class miningBot {
    ...
    register("tick", () => {
      ...
      switch (this.state) {
        case this.MACROSTATES.MINING:
          ...
          if (!TabList.getFooter().includes("Filet O' Fortune") && this.findItemInHotbar("Filet O' Fortune") !== -1) {
        ...
        case this.MACROSTATES.CLICKBOOST:
          ...
          if (this.findItemInHotbar("Rod") !== -1) {
            this.rodSwapAbility = true
          }

          delay = 1

          // if rodswap is enabled
        ...
        case this.MACROSTATES.FILETOFORTUNE:
          ...
          let filetSlot = this.findItemInHotbar("Filet O' Fortune")
          let pumpkinSlot = this.findItemInHotbar("Powder Pumpkin")
    ...
    if (this.Enabled) {
      ...
      switch (this.type) {
        case this.MACROTYPES.MININGBOT:
          ...
          this.rodSwapAbility = ModuleManager.getSetting(this.ModuleName, "Rod Swap Ability")
          if (this.rodSwapAbility) {
            let rodSlot = this.findItemInHotbar("Rod")
            if (rodSlot === -1) {
              this.stopBot("No fishing rod found in hotbar!", true)
              return
            }
          }
  ...
  holdItem(name) {
    let slot = this.findItemInHotbar(name)
