  ...
  aStar(target, smoothing = true, heightCost = 2, begin = undefined, smallArea = false) {
    ...
    while (openSet.size > 0 && !MaxTime.hasReached(this.maxCalculationTime)) {
      ...
      if (current.id === endNode.id) {
        ChatUtils.sendDebugMessage("Generation took " + TimePast.getTimePassed())
        ChatUtils.sendDebugMessage("The path length is: " + path.length)
        ChatUtils.sendDebugMessage("The closest set length is: " + closedSet.size)
...
global.export.aStarRdbt = new PathFinderClass()
