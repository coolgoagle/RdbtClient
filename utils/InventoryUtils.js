...
class InventoryUtilsClass {
  ...
  findSlotByExactName(inv = this.getInventory(), name) {
    ...
    for (let i = 0; i < inv.getSize(); i++) {
      ...
      const itemName = item.getName ? (item.getName().removeFormatting ? item.getName().removeFormatting() : this.stripFormatting(item.getName())) : ""
  ...
  getItemNameInSlot(slot, inv = this.getInventory()) {
    ...
    return item.getName().removeFormatting ? item.getName().removeFormatting() : this.stripFormatting(item.getName())
  }
  ...
  hotbarContains(name, partial = false) {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      const itemName = item.getName().removeFormatting ? item.getName().removeFormatting() : this.stripFormatting(item.getName())

      if (partial ? itemName.toLowerCase().includes(name.toLowerCase()) : itemName.toLowerCase() === name.toLowerCase()) {
        return true
      }
    }
    return false
  ...
  countItems(name, partial = false, inv = this.getInventory()) {
    ...
    for (let i = 0; i < inv.getSize(); i++) {
      ...
      if (partial ? itemName.toLowerCase().includes(nameLower) : itemName.toLowerCase() === nameLower) {
