...
class UtilsClass {
  constructor() {
    ...
    register("chat", Event => {
      ...
      if (msg.toString().includes("is now available!")) {
        CheckEvents("speedboostready")
      }
      if (msg.includes("You used your Mining Speed Boost Pickaxe Ability!") || msg.toString().includes("You used your Maniac Miner Pickaxe Ability!")) {
        CheckEvents("onspeedboost")
      }
      if (msg.includes("Your Mining Speed Boost has expired!") || msg.toString().includes("Your Maniac Miner has expired!")) {
    ...
    register("chat", () => {
      ...
      .setCriteria("&cis empty! Refuel it by talking to a Drill Mechanic!")
      .setContains() 
    /*
  ...
  warnPlayer = (msg = "New Alert!") => {
    ...
    try {
      ...
      ChatUtils.sendModMessage("New Alert! " + msg)
      audio.setVolume(1)
      audio.play()
    } catch (e) {
      ChatLib.chat("&cFailsafe sound assets missing! Try /ct delete RdbtClient!")
    }
  }
    ...
    for (let x = -1; x <= 1; x++) {
      for (let z = -1; z <= 1; z++) {
        for (let y = 0; y <= 1; y++) {
          ...
          boxes.push(new AxisAlignedBB(cords[0] + x - 0.01, cords[1] + y, cords[2] + z - 0.01, cords[0] + x + 1.01, cords[1] + y + ctBlock.type.mcBlock.func_149669_A(), cords[2] + z + 1.01))
        }
      }
    }
    return boxes
  ...
  /**
   * Writes a string directly to a configuration file.
   * @param {string} Name - The name of the configuration file.
   * @param {string} String - The string content to write.
   */
